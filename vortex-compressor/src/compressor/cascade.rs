...
impl CascadingCompressor {
    ...
    fn choose_and_compress(
        ...
        //
        // The scheme's own [`expected_compression_ratio`](Scheme::expected_compression_ratio) is
        // still consulted on the fast path: if the verdict is [`EstimateVerdict::Skip`] the
        // cascade falls through to normal selection. This guards against shape-fragile schemes
        // (BitPacking on signed-negative input, FoR on wrap-overflow spans, the constant family
        // on actually-non-constant data) whose [`matches`](Scheme::matches) returns `true` but
        // whose compress would panic on the canonical input — the cached winner from a prior
        // fragment may not fit the current one. [`DeferredEstimate`] verdicts proceed as cache
        // hits without running the sample compress; if the cached winner turns out to be a poor
        // choice the cascade's accept-only-if-smaller guard still falls back to the canonical
        // encoding.
        //
        // On miss (id not registered, scheme refuses the canonical type via its
        // [`matches`](Scheme::matches) check, the cascade's exclusion rules would otherwise skip
        // it, or `expected_compression_ratio` returns `Skip`) the path falls through to normal
        // selection. A stale or wrong hint never produces an incorrect compressed output; the
        // worst case is one wasted lookup plus one `expected_compression_ratio` call.
            ...
            })
        {
            ...
            // One `ArrayAndStats` is shared between the probe call (expected_compression_ratio)
            // and the compress call. The per-instance stats cache holds the lazy `integer_stats`
            // / `varbinview_stats` so the probe's generation amortises into the compress's.
            // Earlier shape of this code constructed two separate `ArrayAndStats`, paying the
            // stats compute twice and measurably regressing streaming-insert wall-clock vs the
            // pre-gate shape.
            let data = ArrayAndStats::new(array, opts);
            let estimate = frozen.expected_compression_ratio(&data, compress_ctx.clone(), exec_ctx);
            // Verdict::Skip falls through to normal selection (the shape-fragility fence).
            // Verdict::Ratio and Verdict::AlwaysUse take the fast path directly.
            // Deferred::Sample takes the fast path because the sample's only role in normal
            // selection is producing a ratio estimate for comparison against other candidates; we
            // already have a winner via the cache hit, so the comparison is moot and skipping the
            // sample is exactly the C-prime saving — the accept-only-if-smaller guard still
            // catches the case where the scheme's full compress doesn't actually shrink the
            // input.
            // Deferred::Callback falls through because the callback is the actual correctness
            // check, not just a ratio estimate. StringConstantScheme's Callback runs
            // `is_constant` and returns Skip on non-constant data; bypassing the callback in the
            // fast path would land the non-constant array on the constant scheme's compress and
            // corrupt every row to the first scalar.
